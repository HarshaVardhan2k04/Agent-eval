...
class ConversationEngine:
    def __init__(self, llm_client, tool_simulator=None, rag_client=None):
        self.llm = llm_client
        self.tool_simulator = tool_simulator
        self.rag_client = rag_client

    async def _llm_call(self, messages, temperature=None, tools=None):
        temp = temperature if temperature is not None else DEFAULT_AGENT_TEMPERATURE
        msg = await self.llm.chat(messages, tools=tools, temperature=temp, max_tokens=DEFAULT_MAX_TOKENS)
        return msg
    ...
    async def run_single_turn(self, system_prompt, scenario):
        ...
        tools_schema = self.tool_simulator.get_schemas() if self.tool_simulator else None
        response = await self._llm_call(messages, tools=tools_schema)
    ...
    async def run_simulated(self, system_prompt, scenario):
        ...
        if call_direction == "outbound":
            ...
            user_resp = await self._llm_call(user_messages, temperature=DEFAULT_USER_TEMPERATURE)
            ...
            agent_resp = await self._llm_call(agent_messages)
